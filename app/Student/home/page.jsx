...
export default function StudentHome() {
    ...
    return (
        ...
        <Container>
            ...
            <Heading size="md" mb={6} color="fg.muted">تحب تكمل؟</Heading>
            <SimpleGrid columns={{ base: 1, sm: 2, lg: 3 }} gap={6} mb={10}>
            {courses.map((i) => (
                <Box key={i} bg="bg.subtle" borderRadius="2xl" overflow="hidden" shadow="sm" border="1px solid #E2E8F0">
                <Box position="relative">
                    <Image src={lessonImage} alt="كيمياء" w="100%" />
                    <Badge position="absolute" top={3} left={3} bg="white/80" borderRadius="full" px={2}  color="fg.muted">
                    350 مشاهدة
                    </Badge>
                </Box>
                <VStack align="flex-start" p={4} gap={1}>
                    <Text fontWeight="bold" fontSize="sm" lineHeight="tall"  color="fg.muted">
            ...
            {teachers.map((i) => (
                <Box key={i} bg="bg.subtle" p={4} borderRadius="2xl" border="1px solid #009EDB">
